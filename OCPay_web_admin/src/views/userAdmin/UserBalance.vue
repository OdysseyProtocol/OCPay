<template>
  <div>
    <h3>用户货币余额  </h3>
    <hr/>
    <el-form  :inline="true" :model="formInline" class="demo-form-inline wallet">
        <el-col :span="22">
            <el-form-item label="所属平台">
                 <el-select v-model="formInline.platform" placeholder="请选择平台类型" @change="onSubmit">
                     <el-option label="所有" value=""></el-option>
                        <el-option v-for="(item,index) in platform" :key="index" :label="item.merchantName" :value="item.id"></el-option>
                    
                </el-select>
            </el-form-item>
            <el-form-item label="交易时间">
                <el-col :span="11">
                <el-date-picker type="date" placeholder="选择日期" v-model="formInline.date" style="width: 100%;" @change="onSubmit"></el-date-picker>
                </el-col>
                <el-col class="line" :span="2">-</el-col>
                <el-col :span="11">
                <el-date-picker type="date" placeholder="选择日期" v-model="formInline.date1" style="width: 100%;" @change="onSubmit"></el-date-picker>
                </el-col>
            </el-form-item>
             <!-- <el-form-item label="用户状态">
                 <el-select v-model="formInline.userStatus" placeholder="请选择用户状态">
                     <el-option label="所有" value=""></el-option>
                     <el-option label="正常" value="0"></el-option>
                    
                </el-select>
            </el-form-item> -->
            <el-form-item label="操作状态">
                 <el-select v-model="formInline.transferStatus" placeholder="请选择用户状态" @change="onSubmit">
                     <el-option label="所有" value=""></el-option>
                     <el-option label="无转账操作" value="0"></el-option>
                    <el-option label="正在转账" value="1"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item  label="用户id">
                <el-input v-model="formInline.userid" placeholder="请输入用户id" @change="onSubmit"></el-input>
            </el-form-item>  
            <el-form-item  label="代币名称">
                <el-input v-model="formInline.coinName" placeholder="请输入代币名称" @change="onSubmit"></el-input>
            </el-form-item>  
        </el-col>
        <el-col :span="2">
            <el-button type="primary" @click="onSubmit">查 询</el-button>
            <el-button type="primary" @click="onSubmitForm">清空</el-button>
        </el-col>
        <div style="clear:both;"></div>
    </el-form>  
    <hr/>
    <div class="table">
        <el-table
            :data="tableData"
            border
            max-height="550"
            style="width: 100%">
            <!-- <el-table-column
            prop="id"
            label="id">
            </el-table-column> -->
            <el-table-column
            width="100px"
            label="序号" fixed="left">
                <template slot-scope="scope1">
                    
					{{scope1.$index+1+10*(currentPage-1)}}
				</template>
            </el-table-column>
              <el-table-column
            prop="userid"
            label="用户id" width="150px">
            </el-table-column>
            <el-table-column
            label="第三方平台" width="150px">
                <template slot-scope="scope">
                     <span v-for="(item,index) in platform" :key="index" v-if="item.id==scope.row.merchantId">{{item.merchantName}} </span>
                </template>
            </el-table-column>
            <el-table-column
            prop="coinName"
            label="货币名称" width="150px">
            </el-table-column>
            <el-table-column
            prop="coinBalance"
            label="代币余额" width="150px">
            </el-table-column>
            <el-table-column
            prop="showBalance"
            label="账户余额" width="150px">
            </el-table-column>
             <el-table-column
            prop="totalAmount"
            label="充值总额" width="150px">
            </el-table-column>
            <!-- <el-table-column
            label="用户状态">
                <template slot-scope="scope">
                   <span v-if="scope.row.userStatus==0">正常 </span>
                    <span v-if="scope.row.userStatus!=0">异常</span>
                   
                </template>
            </el-table-column> -->
            <el-table-column
            label="正在进行的操作" width="150px">
            <template slot-scope="scope">
                   <span v-if="scope.row.transferStatus==0">无转账操作 </span>
                    <span v-if="scope.row.transferStatus==1">正在转账</span>
                   
                </template>
            </el-table-column>
            <el-table-column
            label="创建时间" width="170px">
                <template slot-scope="scope">
                    <span >{{ scope.row.createTime | time }}</span>
                </template>
            </el-table-column>
             <el-table-column
            label="最后交易时间" width="170px">
                <template slot-scope="scope">
                    <span >{{ scope.row.lastTradingTime | time }}</span>
                </template>
            </el-table-column>
           
             <el-table-column
            label="提到总账时间" width="170px">
                <template slot-scope="scope">
                    <span >{{ scope.row.toWalletTime | time }}</span>
                </template>
            </el-table-column>
            <!-- <el-table-column
            label="最后交易时间">
                <template slot-scope="scope">
                    <span >{{ scope.row.lastTradingTime | time }}</span>
                </template>
            </el-table-column> -->
           
            
        </el-table>
    </div>
     <div class="block" style="text-align:center;margin-top:20px;">
        <el-pagination
          background
          @current-change="handleCurrentChange"
          :current-page="currentPage"
          :page-size="pagesize"
          layout=" prev, pager, next"
          :total="totalCount">
        </el-pagination>
      </div>
    
  </div>
</template>

<script>
import { userBalance,allPlatform } from '../../api/api';
export default {
    data() {
        return {
            formInline: {
                platform:'',
                date:'',
                date1:'',
                userStatus:'',
                transferStatus:'',
                coinName:'',
                userid:''
            
            },
            tableData: [],
            platform:[],
             //默认每页数据量
            pagesize: 10,
            //当前页码
            currentPage: 1,
            //默认数据总数
            totalCount: 20,
            

        };
    },
    mounted(){
    
        this.getcoin(this.currentPage)
        
        this.tableData.lastTradingTime=new Date(this.tableData.lastTradingTime);
        allPlatform().then(data=>{
            this.platform=data.data;
        })
    },
    methods: {
        onSubmitForm(){
            this.formInline.platform=''
            this.formInline.state=''
            this.formInline.date=''
            this.formInline.date1=''
             this.formInline.transferStatus=''
              this.formInline.userid=''
               this.formInline.coinName=''
            this.getcoin(this.currentPage);
        },
        getcoin(num){
            let date=new Date(this.formInline.date).getTime();
            let date1=new Date(this.formInline.date1).getTime();
            let para='?pageNum='+num
            if(this.formInline.platform!=''){
                 para=para+'&merchantId='+this.formInline.platform
            }
             if(this.formInline.date!=''){
                 para=para+'&beginTime='+date
            }
            if(this.formInline.date1!=''){
                 para=para+'&endTime='+date1
            }
            if(this.formInline.transferStatus!=''){
                 para=para+'&transferStatus='+this.formInline.transferStatus
            }
             if(this.formInline.userid!=''){
                 para=para+'&userid='+this.formInline.userid
            }
             if(this.formInline.coinName!=''){
                 para=para+'&coinName='+this.formInline.coinName 
            }
            userBalance(para).then(data=>{
                this.tableData=data.data.list;
                this.pagesize=data.data.per;
                this.totalCount=data.data.per*data.data.pages;
            })
        },
         handleCurrentChange(val){
            this.currentPage=val;
            this.getcoin(this.currentPage);
        },
        onSubmit() {

           this.getcoin(this.currentPage);
        }
       
    }
}

</script>
<style>
.wallet{
    padding: 20px;
}
.table{
    margin-top: 30px;
}
.line{
    text-align: right;
    padding-right: 5px;
}
.add{
    padding-left: 5px;
}
.add1{
    padding-left: 15px;
}
.secret{
    text-align: right;
    padding-right: 5px;
}
.erweima{
    padding-left: 20px;
}
.el-button+.el-button{
    margin-left: 2px;
}
</style>
